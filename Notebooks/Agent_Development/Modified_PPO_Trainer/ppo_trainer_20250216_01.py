...
def prep_llm_input(env_cfg, obs, team_id, enemy_team_id, enemy_spawn_location):
...
class ModifiedPPOTrainer(Trainer):
    ...
    def __init__(
        ...
        self.model_2, self.optimizer_2 = accelerator.prepare(self.model_2, self.optimizer_2) # edit it later
    ...
    def train(self):
        ...
        for game_number in range(1, num_games_to_train + 1):
            ...
            while True:
                ...
                player_0_llm_input = prep_llm_input(env_cfg, obs_all['player_0'], 0, 1, player_1_spawn_location)
                player_1_llm_input = prep_llm_input(env_cfg, obs_all['player_1'], 1, 0, player_0_spawn_location)
                ...
                with torch.no_grad():
                    ...
                    del player_0_logits, player_0_all_logprob, player_0_llm_input, player_0_input, player_0_tokens
                    ...
                    # _, player_0_score, _ = get_reward(
                    #     reward_model, player_0_postprocessed_query_response, processing_class.pad_token_id, player_0_context_length
                    # )

                    player_0_completion_ids = player_0_postprocessed_query_response[:, player_0_context_length:]
                    player_0_completions = self.processing_class.decode(player_0_completion_ids, skip_special_tokens=True)
                    ...
                    for i, reward_func in enumerate(self.reward_functions):
                        reward = reward_func(player_0_completions)
                    ...
                    del (
                        player_0_full_value, player_0_unwrapped_model, player_0_unwrapped_value_model, player_0_completion_ids, player_0_func_rewards
                    )
                    torch.cuda.empty_cache()
                    gc.collect()
                    ...
                    del player_1_logits, player_1_all_logprob, player_1_llm_input, player_1_input, player_1_tokens
                    torch.cuda.empty_cache()
                    gc.collect()
                    ...
                    # _, player_1_score, _ = get_reward(
                    #     reward_model, player_1_postprocessed_query_response, processing_class.pad_token_id, player_1_context_length
                    # )

                    player_1_completion_ids = player_1_postprocessed_query_response[:, player_1_context_length:]
                    player_1_completions = self.processing_class.decode(player_1_completion_ids, skip_special_tokens=True)

                    player_1_func_rewards = torch.zeros(1, len(self.reward_functions), device=device)

                    for i, reward_func in enumerate(self.reward_functions):
                        reward = reward_func(player_1_completions)
                    ...
                    del (
                        player_1_full_value, player_1_unwrapped_model, player_1_unwrapped_value_model, player_1_completion_ids, player_1_func_rewards
                    )
                    torch.cuda.empty_cache()
                    gc.collect()
                    ...
                    for mini_batch_start in range(0, args.local_batch_size, args.local_mini_batch_size):
                        ...
                        for micro_batch_start in range(0, args.local_mini_batch_size, args.per_device_train_batch_size):
                            with accelerator.accumulate(model):
                                ...
                                player_0_mb_advantage = player_0_advantages[micro_batch_inds]
                                player_0_mb_responses = player_0_response[micro_batch_inds]
                                player_0_mb_query_responses = player_0_query_response[micro_batch_inds]
                                player_0_mb_logprobs = player_0_logprob[micro_batch_inds]
                                player_0_mb_return = player_0_returns[micro_batch_inds]
                                player_0_mb_values = player_0_value[micro_batch_inds]
                                ...
                                player_0_new_logprobs = torch.masked_fill(
                                    player_0_new_logprobs, player_0_padding_mask[micro_batch_inds], INVALID_LOGPROB
                                )
                                player_0_vpred = player_0_vpred_temp[:, player_0_context_length - 1 : -1].squeeze(-1)
                                player_0_vpred = torch.masked_fill(player_0_vpred, player_0_padding_mask_p1[micro_batch_inds], 0)
                                ...
                                player_0_vf_loss = 0.5 * masked_mean(player_0_vf_loss_max, ~player_0_padding_mask_p1[micro_batch_inds])
                                player_0_vf_clipfrac = masked_mean(
                                    (player_0_vf_losses2 > player_0_vf_losses1).float(), ~player_0_padding_mask_p1[micro_batch_inds]
                                )
                                ...
                                player_0_pg_loss = masked_mean(player_0_pg_loss_max, ~player_0_padding_mask[micro_batch_inds])
                            ...
                            with accelerator.accumulate(model_2):
                                micro_batch_end = micro_batch_start + args.per_device_train_batch_size
                                micro_batch_inds = mini_batch_inds[micro_batch_start:micro_batch_end]
                                player_1_mb_advantage = player_1_advantages[micro_batch_inds]
                                player_1_mb_responses = player_1_response[micro_batch_inds]
                                player_1_mb_query_responses = player_1_query_response[micro_batch_inds]
                                player_1_mb_logprobs = player_1_logprob[micro_batch_inds]
                                player_1_mb_return = player_1_returns[micro_batch_inds]
                                player_1_mb_values = player_1_value[micro_batch_inds]
                                ...
                                player_1_new_logprobs = torch.masked_fill(
                                    player_1_new_logprobs, player_1_padding_mask[micro_batch_inds], INVALID_LOGPROB
                                )
                                player_1_vpred = player_1_vpred_temp[:, player_1_context_length - 1 : -1].squeeze(-1)
                                player_1_vpred = torch.masked_fill(player_1_vpred, player_1_padding_mask_p1[micro_batch_inds], 0)
                                ...
                                player_1_vf_loss = 0.5 * masked_mean(player_1_vf_loss_max, ~player_1_padding_mask_p1[micro_batch_inds])
                                player_1_vf_clipfrac = masked_mean(
                                    (player_1_vf_losses2 > player_1_vf_losses1).float(), ~player_1_padding_mask_p1[micro_batch_inds]
                                )
                                ...
                                player_1_pg_loss = masked_mean(player_1_pg_loss_max, ~player_1_padding_mask[micro_batch_inds])
                                ...
                                with torch.no_grad():
                                    player_1_pg_clipfrac = masked_mean(
                                        (player_1_pg_losses2 > player_1_pg_losses).float(), ~player_1_padding_mask[micro_batch_inds]
                                    )
                        ...
                        del (
                            ...
                            player_0_pg_loss, player_0_loss, player_0_pg_clipfrac, player_0_prob_dist, player_0_entropy, player_0_approxkl, player_0_mb_return,
                        ...
                        del (
                            ...
                            player_1_pg_loss, player_1_loss, player_1_pg_clipfrac, player_1_prob_dist, player_1_entropy, player_1_approxkl, player_1_mb_return,
                ...
                del player_0_kl, player_0_mean_kl, player_0_mean_entropy, player_0_mean_non_score_reward, player_0_score, player_0_metrics, player_0_non_score_reward
                ...
                del player_1_kl, player_1_mean_kl, player_1_mean_entropy, player_1_mean_non_score_reward, player_1_score, player_1_metrics, player_1_non_score_reward
                torch.cuda.empty_cache()
                gc.collect()
                ...
                obs_all, reward, terminated, truncated, match_info = env.step({
                    "player_0": actions0,
                    "player_1": actions1
                })
