...
class ModifiedPPOTrainer(Trainer):
    ...
    def __init__(
        ...
        if self.args.stop_token and self.args.stop_token_id:
            ...
            self.policy_model.generation_config.eos_token_id = self.stop_token_id = self.args.stop_token_id  # None or int
            self.policy_model_2.generation_config.eos_token_id = self.stop_token_id = self.args.stop_token_id  # None or int
        ...
        if not is_peft_available() and peft_config is not None:
            ...
            if self.args.bf16 and getattr(self.policy_model_2, "is_loaded_in_4bit", False):
        ...
        #     },
        # )
        plugin = GradientAccumulationPlugin(sync_with_dataloader=False, num_steps=self.args.gradient_accumulation_steps, sync_each_batch=True)
    ...
    def train(self):
        ...
        for game_number in range(1, self.num_games_to_train + 1):
            ...
            while game_ended is not True:
                ...
                = self.response_processing(
                    self.model, player_0_llm_input, player_0_reward_score, player_0_match_won, player_0_match_lost, player_0_game_won, player_0_game_lost
                )
                ...
                self.train_step(
                    self.model, self.optimizer, player_0_logprob, player_1_logprob, player_0_sequence_length, player_0_sequence_length_p1, player_0_padding_mask_p1, player_0_score,
                    player_0_response, player_0_value, player_0_padding_mask, player_0_query_response, player_0_context_length
                )
                ...
                self.train_step(
                    ...
                    player_1_response, player_1_value, player_1_padding_mask, player_1_query_response, player_1_context_length
                )
                ...
                if self.control_2.should_save:
                    self._save_checkpoint(self.model_2, trial=None)
        ...
        if self.control_2.should_save:
            self._save_checkpoint(self.model_2, trial=None, metrics=None)
            self.control_2 = self.callback_handler_2.on_save(self.args, self.state_2, self.control_2)
    ...
    def response_processing(self, model, llm_input, reward_score, match_won, match_lost, game_won, game_lost):
        ...
        input = "".join([answer_format, game_rules, llm_input])
        tokens = self.processing_class(input, return_tensors="pt", max_length=False, truncation=False, padding=False).to(self.device)
        ...
        with torch.no_grad():
            ...
            with unwrap_model_for_generation(
                ...
                gen_output = unwrapped_model.policy.generate(
                    **tokens,
            ...
            logits = torch.stack(gen_output.scores, 1)
    ...
    def train_step(
            self, model, optimizer, my_logprob, enemy_logprob, sequence_length, sequence_length_p1, padding_mask_p1, score, response, value, padding_mask, query_response, context_length
        ):
        args = self.args
        ...
        with accelerator.accumulate(model):
            output, vpred_temp = forward(model, query_response, self.processing_class.pad_token_id)
