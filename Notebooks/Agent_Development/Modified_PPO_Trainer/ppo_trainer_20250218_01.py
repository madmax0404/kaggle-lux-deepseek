...
class PolicyAndValueWrapper(nn.Module):
    def __init__(self, policy, value_model) -> None:
        ...
        # This should be the heavy module; ensure it is defined.
        self.critic_backbone = getattr(value_model, value_model.base_model_prefix)

    def forward(self, **kwargs):
        # Assume that kwargs contains tensors "input_ids" and optionally "attention_mask"
        input_ids = kwargs["input_ids"]
        attention_mask = kwargs.get("attention_mask", None)

        # Define a function that calls the critic backbone.
        def backbone_forward(input_ids, attention_mask=None):
            # Create a new kwargs dict so that checkpoint only receives tensors.
            new_kwargs = kwargs.copy()
            new_kwargs["input_ids"] = input_ids
            if attention_mask is not None:
                new_kwargs["attention_mask"] = attention_mask
            return self.critic_backbone(**new_kwargs)

        # Use checkpoint: note that all tensor arguments must be passed explicitly.
        if attention_mask is not None:
            output = checkpoint.checkpoint(backbone_forward, input_ids, attention_mask)
        else:
            output = checkpoint.checkpoint(backbone_forward, input_ids)

        # Use the value head on the final hidden state.
        logits = self.value_model.score(output.hidden_states[-1])
        # The policy part still runs normally.
...
class ModifiedPPOTrainer(Trainer):
    ...
    def __init__(
        ...
        #     },
        # )
        plugin = GradientAccumulationPlugin(sync_with_dataloader=False, num_steps=args.gradient_accumulation_steps, sync_each_batch=True)
        ...
        # accelerator.state.deepspeed_plugin.deepspeed_config["distributed_type"] = "NO"
        # accelerator.state.deepspeed_plugin.deepspeed_config["train_batch_size"] = 1

        # accelerator_2 = Accelerator(mixed_precision="bf16", gradient_accumulation_plugin=plugin, deepspeed_plugin=ds_plugin)
    ...
    def train(self):
        ...
        # accelerator_2 = self.accelerator_2
        ...
        for game_number in range(1, num_games_to_train + 1):
            ...
            while game_ended is not True:
                ...
                player_0_tokens = processing_class(player_0_input, return_tensors="pt", max_length=False, truncation=False, padding=False).to(device)
                player_1_tokens = processing_class(player_1_input, return_tensors="pt", max_length=False, truncation=False, padding=False).to(device)
                ...
                with accelerator.accumulate(model):
                    player_0_output, player_0_vpred_temp = forward(model, player_0_query_response, processing_class.pad_token_id)
                    ...
                    player_0_new_logprobs = torch.gather(player_0_new_all_logprobs, 2, player_0_response.unsqueeze(-1)).squeeze(-1)
                    ...
                    player_0_vpredclipped = torch.clamp(
                        player_0_vpred,
                        player_0_value - args.cliprange_value,
                        player_0_value + args.cliprange_value,
                    )
                    player_0_vf_losses1 = torch.square(player_0_vpred - player_0_returns)
                    player_0_vf_losses2 = torch.square(player_0_vpredclipped - player_0_returns)
                    ...
                    # )
                    player_0_logprobs_diff = player_0_new_logprobs - player_0_logprob
                    player_0_ratio = torch.exp(player_0_logprobs_diff)
                    player_0_pg_losses = -player_0_advantages * player_0_ratio
                    player_0_pg_losses2 = -player_0_advantages * torch.clamp(player_0_ratio, 1.0 - args.cliprange, 1.0 + args.cliprange)
                ...
                del (
                    ...
                    player_0_vf_losses1, player_0_vf_losses2, player_0_vf_loss, player_0_logprobs_diff, player_0_ratio, player_0_pg_losses, player_0_pg_losses2,
                ...
                with accelerator.accumulate(model_2):
                    player_1_output, player_1_vpred_temp = forward(model_2, player_1_query_response, processing_class.pad_token_id)
                    ...
                    player_1_new_logprobs = torch.gather(player_1_new_all_logprobs, 2, player_1_response.unsqueeze(-1)).squeeze(-1)
                    ...
                    player_1_vpredclipped = torch.clamp(
                        player_1_vpred,
                        player_1_value - args.cliprange_value,
                        player_1_value + args.cliprange_value,
                    )
                    player_1_vf_losses1 = torch.square(player_1_vpred - player_1_returns)
                    player_1_vf_losses2 = torch.square(player_1_vpredclipped - player_1_returns)
                    ...
                    # )
                    player_1_logprobs_diff = player_1_new_logprobs - player_1_logprob
                    player_1_ratio = torch.exp(player_1_logprobs_diff)
                    player_1_pg_losses = -player_1_advantages * player_1_ratio
                    player_1_pg_losses2 = -player_1_advantages * torch.clamp(player_1_ratio, 1.0 - args.cliprange, 1.0 + args.cliprange)
                ...
                del (
                    ...
                    player_1_vf_losses1, player_1_vf_losses2, player_1_vf_loss, player_1_logprobs_diff, player_1_ratio, player_1_pg_losses, player_1_pg_losses2,
    ...
    def get_action_from_answer(self, answer):
        ...
        for i, unit_answer in enumerate(answer):
            if "Unit" in unit_answer and ":" in unit_answer:
